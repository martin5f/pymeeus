...
from pymeeus_oo.calculation.ellipsoid import IAU76
...
def test_earth_constructor():
    ...
    epoch = Epoch(2020, 12, 20)
    a = Earth(epoch)
    assert abs(a.ellipsoid()._a - 6378137.0) < TOL, \
        "ERROR: 1st constructor test, 'a' value doesn't match"

    assert abs(a.ellipsoid()._f - (1.0/298.257223563)) < TOL, \
        "ERROR: 2nd constructor test, 'f' value doesn't match"

    assert abs(a.ellipsoid()._omega - 7292115e-11) < TOL, \
        "ERROR: 3rd constructor test, 'omega' value doesn't match"

    a = Earth(epoch, ellipsoid=IAU76)
    assert abs(a.ellipsoid()._a - 6378140.0) < TOL, \
        "ERROR: 4th constructor test, 'a' value doesn't match"

    assert abs(a.ellipsoid()._f - (1.0/298.257)) < TOL, \
        "ERROR: 5th constructor test, 'f' value doesn't match"

    assert abs(a.ellipsoid()._omega - 7.292114992e-5) < TOL, \
...
def test_earth_rho():
    ...
    epoch = Epoch(2020, 12, 20)
    e = Earth(epoch)
    assert abs(e.ellipsoid().rho(0.0) - 1.0) < TOL, \
...
def test_earth_rho_sinphi():
    ...
    epoch = Epoch(2020, 12, 20)
    e = Earth(epoch, ellipsoid=IAU76)
    assert abs(round(e.ellipsoid().rho_sinphi(lat, 1706), 6) - 0.546861) < TOL, \
...
def test_earth_rho_cosphi():
    ...
    epoch = Epoch(2020, 12, 20)
    e = Earth(epoch, ellipsoid=IAU76)
    assert abs(round(e.ellipsoid().rho_cosphi(lat, 1706), 6) - 0.836339) < TOL, \
...
def test_earth_rp():
    ...
    epoch = Epoch(2020, 12, 20)
    e = Earth(epoch, ellipsoid=IAU76)
    assert abs(round(e.ellipsoid().rp(42.0), 1) - 4747001.2) < TOL, \
...
def test_earth_rm():
    ...
    epoch = Epoch(2020, 12, 20)
    e = Earth(epoch, ellipsoid=IAU76)
    assert abs(round(e.ellipsoid().rm(42.0), 1) - 6364033.3) < TOL, \
...
def test_earth_linear_velocity():
    ...
    epoch = Epoch(2020, 12, 20)
    e = Earth(epoch, ellipsoid=IAU76)
    assert abs(round(e.ellipsoid().linear_velocity(42.0), 2) - 346.16) < TOL, \
...
def test_earth_distance():
    ...
    dist, error = e.ellipsoid().distance(lon1, lat1, lon2, lat2)
...
def test_earth_orbital_elements_mean_equinox():
    ...
    l, a, e, i, ome, arg = Earth(epoch).orbital_elements_mean_equinox()
...
def test_earth_orbital_elements_j2000():
    ...
    epoch = Epoch(2065, 6, 24.0)
    l, a, e, i, ome, arg = Earth(epoch).orbital_elements_j2000()
...
def test_earth_passage_nodes():
    ...
    time, r = Earth(epoch).passage_nodes()
    y, m, d = time.get_date()
    d = round(d, 1)
    r = round(r, 4)
