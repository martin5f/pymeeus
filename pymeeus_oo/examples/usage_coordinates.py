...
import pymeeus_oo.calculation.coordinates
...
def main():
    ...
    e0 = pymeeus_oo.calculation.coordinates.mean_obliquity(1987, 4, 10)
    print(
        "The mean angle between Earth rotation axis and ecliptic axis for "
        + "1987/4/10 is:"
    )
    print_me("Mean obliquity", e0.dms_str(n_dec=3))  # 23d 26' 27.407''
    epsilon = pymeeus_oo.calculation.coordinates.true_obliquity(1987, 4, 10)
    ...
    epsilon = pymeeus_oo.calculation.coordinates.true_obliquity(2018, 7, 29)
    ...
    dpsi = pymeeus_oo.calculation.coordinates.nutation_longitude(1987, 4, 10)
    print_me("Nutation in longitude", dpsi.dms_str(n_dec=3))  # 0d 0' -3.788''
    print("Nutation correction in obliquity for 1987/4/10:")
    depsilon = pymeeus_oo.calculation.coordinates.nutation_obliquity(1987, 4, 10)  # 0d 0' 9.443''
    ...
    alpha, delta = pymeeus_oo.calculation.coordinates.precession_equatorial(
    ...
    lon, lat = pymeeus_oo.calculation.coordinates.precession_ecliptical(start_epoch, final_epoch, lon0, lat0)
    ...
    dist = 2.64
    vel = -7.6
    alpha, delta = pymeeus_oo.calculation.coordinates.motion_in_space(ra, dec, dist, vel, pm_ra, pm_dec, -1000.0)
    ...
    lon, lat = pymeeus_oo.calculation.coordinates.equatorial2ecliptical(ra, dec, epsilon)
    ...
    lat = Angle(6.68417)
    epsilon = Angle(23.4392911)
    ra, dec = pymeeus_oo.calculation.coordinates.ecliptical2equatorial(lon, lat, epsilon)
    ...
    theta0 += delta
    h = theta0 - lon - ra
    azi, ele = pymeeus_oo.calculation.coordinates.equatorial2horizontal(h, dec, lat)
    ...
    ele = Angle(15.1249)
    lat = Angle(38, 55, 17)
    h, dec = pymeeus_oo.calculation.coordinates.horizontal2equatorial(azi, ele, lat)
    ...
    dec = Angle(-14, 43, 8.2)
    lon, lat = pymeeus_oo.calculation.coordinates.equatorial2galactic(ra, dec)
    ...
    lon = Angle(12.9593)
    lat = Angle(6.0463)
    ra, dec = pymeeus_oo.calculation.coordinates.galactic2equatorial(lon, lat)
    ...
    lat = Angle(51.0)
    epsilon = Angle(23.44)
    lon1, lon2, i = pymeeus_oo.calculation.coordinates.ecliptic_horizon(sidereal_time, lat, epsilon)
    print_me(
    ...
    dec = Angle(23.44)
    lat = Angle(40.0)
    j = pymeeus_oo.calculation.coordinates.diurnal_path_horizon(dec, lat)
    print_me(
    ...
    rising, transit, setting = pymeeus_oo.calculation.coordinates.times_rise_transit_set(
        longitude,
        latitude,
        alpha1,
        delta1,
    ...
    true_elevation = pymeeus_oo.calculation.coordinates.refraction_apparent2true(apparent_elevation)
    print_me(
    ...
    apparent_elevation = pymeeus_oo.calculation.coordinates.refraction_true2apparent(true_elevation)
    print_me(
    ...
    sep_ang = pymeeus_oo.calculation.coordinates.angular_separation(alpha1, delta1, alpha2, delta2)
    print_me(
    ...
    a = pymeeus_oo.calculation.coordinates.minimum_angular_separation(
        alpha1_1,
        delta1_1,
        alpha1_2,
        delta1_2,
    ...
    delta2 = Angle(-11, 9, 41.0)
    pos_ang = pymeeus_oo.calculation.coordinates.relative_position_angle(alpha1, delta1, alpha2, delta2)
    ...
    pc = pymeeus_oo.calculation.coordinates.planetary_conjunction(alpha1_list, delta1_list, alpha2_list,
                                                                  delta2_list)
    ...
    pc = pymeeus_oo.calculation.coordinates.planet_star_conjunction(alpha_list, delta_list, alpha_star,
                                                                    delta_star)
    ...
    alpha_list = [alpha_1, alpha_2, alpha_3, alpha_4, alpha_5]
    delta_list = [delta_1, delta_2, delta_3, delta_4, delta_5]
    n = pymeeus_oo.calculation.coordinates.planet_stars_in_line(alpha_list, delta_list, alpha_star1, delta_star1,
    ...
    psi, omega = pymeeus_oo.calculation.coordinates.straight_line(alpha1, delta1, alpha2, delta2, alpha3, delta3)
    ...
    d = pymeeus_oo.calculation.coordinates.circle_diameter(alpha1, delta1, alpha2, delta2, alpha3, delta3)
    print_me(
    ...
    app_alpha, app_delta = pymeeus_oo.calculation.coordinates.apparent_position(epoch, alpha, delta, sun_lon)
    ...
    arg0 = Angle(151.4486)
    lon0 = Angle(45.7481)
    i1, arg1, lon1 = pymeeus_oo.calculation.coordinates.orbital_equinox2equinox(epoch0, epoch, i0, arg0, lon0)
    ...
    e, v = pymeeus_oo.calculation.coordinates.kepler_equation(eccentricity, mean_anomaly)
    print_me("Eccentric anomaly, Case #1", round(e(), 6))  # 5.554589
    print_me("True anomaly, Case #1", round(v(), 6))  # 6.139762
    e, v = pymeeus_oo.calculation.coordinates.kepler_equation(0.99, Angle(0.2, radians=True))
    ...
    time, r = pymeeus_oo.calculation.coordinates.passage_nodes_elliptic(omega, e, a, t)
    y, m, d = time.get_date()
    d = round(d, 2)
    ...
    time, r = pymeeus_oo.calculation.coordinates.passage_nodes_parabolic(omega, q, t, ascending=False)
    y, m, d = time.get_date()
    d = round(d, 2)
    ...
    angle = pymeeus_oo.calculation.coordinates.phase_angle(sun_dist, earth_dist, sun_earth_dist)
    print_me("Phase angle", round(angle, 2))  # 72.96
    # Now, let's compute the illuminated fraction of the disk
    k = pymeeus_oo.calculation.coordinates.illuminated_fraction(sun_dist, earth_dist, sun_earth_dist)
