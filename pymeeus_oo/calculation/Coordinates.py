...
from pymeeus_oo.calculation.base import TOL, iint
from pymeeus_oo.parameters.coordinates_params import NUTATION_ARG_TABLE, NUTATION_SINE_COEF_TABLE, \
...
def vsop_pos(epoch, vsop_l, vsop_b, vsop_r):
    ...
    lon = vsop_pos_terms(t, vsop_l)
    lon = Angle(lon, radians=True).to_positive()

    lat = vsop_pos_terms(t, vsop_b)
    lat = Angle(lat, radians=True)

    r = vsop_pos_terms(t, vsop_r)

    return (lon, lat, r)


def vsop_pos_terms(t, vsop_term_table):
    sum_list = []
    for i in range(len(vsop_term_table)):
        s = 0.0
        for k in range(len(vsop_term_table[i])):
            s += vsop_term_table[i][k][0] * cos(vsop_term_table[i][k][1] + vsop_term_table[i][k][2] * t)
        sum_list.append(s)
    sum = 0.0
    # Sum the terms, while multiplying by 't' at the same time
    for i in range(len(sum_list) - 1, 0, -1):
        sum = (sum + sum_list[i]) * t
...
def apparent_vsop_pos(epoch, vsop_l, vsop_b, vsop_r, nutation=True):
    ...
    :param epoch: Epoch to compute the position, give0n as an :class:`Epoch`
        object
    :type epoch: :py:class:`Epoch`
    :param vsop_l: Table of VSOP87 terms for the heliocentric longitude
    :type vsop_l: list
